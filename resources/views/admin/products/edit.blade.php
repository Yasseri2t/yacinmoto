                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="price" class="form-control" required
                                value="{{ old('price', $product->price) }}" step="50">
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="section" class="form-select">
                                ...
                                <option value="pieces" {{ $product->section == 'pieces' ? 'selected' : '' }}>⚙️ Pièces
                                </option>
                                <option value="carenage" {{ $product->section == 'carenage' ? 'selected' : '' }}>🛡️
                                    Carénage</option>
                                <option value="moteur" {{ $product->section == 'moteur' ? 'selected' : '' }}>⚡ Moteur &
                                    Électrique</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="compatible_models_select[]" class="form-select" multiple id="motoSelect"
                                ...
                                @foreach ($motoTypes as $m)
                                    <option value="{{ $m->name }}"
                                        {{ in_array($m->name, $currentModels) ? 'selected' : '' }}>{{ $m->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            @if ($product->image)
                                <div class="mb-2">
                                    <img src="{{ $product->image }}" id="mainPreview"
                                        style="width:80px;height:80px;object-fit:contain;border-radius:8px;border:2px solid #ff6b00;background:#f8f8f8;padding:4px;">
                                </div>
                            @endif
                            <input type="url" name="image" class="form-control"
                                value="{{ old('image', $product->image) }}" placeholder="https://res.cloudinary.com/..."
                                id="imgInput">
                            ...
                            @if ($product->images->count())
                                <div class="d-flex gap-2 mb-2 flex-wrap">
                                    @foreach ($product->images as $img)
                                        <img src="{{ $img->url }}"
                                            style="width:70px;height:70px;object-fit:contain;border-radius:8px;border:2px solid #ff6b00;background:#f8f8f8;padding:3px;">
                                    @endforeach
                                </div>
                            @endif
                            <textarea name="extra_images" class="form-control" rows="4" id="extraInput"
                                placeholder="Collez une URL par ligne...">{{ $product->images->pluck('url')->implode("\n") }}</textarea>
                            <small class="text-muted">Une URL par ligne — remplace les photos existantes</small>
                        ...
                        <div class="col-12 d-flex gap-4">
                            ...
                            <div class="form-check">
                                <input type="checkbox" name="is_piece_of_day" class="form-check-input" id="pod"
                                    {{ $product->is_piece_of_day ? 'checked' : '' }}>
