    ...
    <div class="row g-4">
        <div class="col-md-5">
            ...
            <img src="{{ Storage::url($product->image) }}" class="img-fluid rounded shadow"
                style="width:100%;max-height:420px;object-fit:contain;background:#f8f8f8;padding:10px;" alt="{{ $product->name }}">
            @else
            <div class="rounded d-flex align-items-center justify-content-center" style="height:350px;background:#f0f0f0;">
        ...
        <div class="col-md-7">
            ...
            @if($product->brand)<p class="mb-1"><strong>Marque:</strong> {{ $product->brand }}</p>@endif
            @if($product->compatible_models)<p class="mb-1"><strong>Compatible:</strong> <span style="color:var(--primary)">{{ $product->compatible_models }}</span></p>@endif
            @if($product->description)<p class="mt-3 text-muted">{{ $product->description }}</p>@endif
            ...
            <div class="d-flex gap-2">
                ...
                <button onclick="addToCartAndOrder({{ $product->id }}, '{{ addslashes($product->name) }}', {{ $product->price }}, '{{ $product->image ? Storage::url($product->image) : '' }}')"
                    class="btn text-white fw-700 flex-grow-1 py-3" style="background:var(--primary);border-radius:10px;font-size:1rem;">
            ...
            <div class="card border-0 shadow-sm" style="border-radius:12px;">
                <div class="card-body p-4">
                    ...
                    <form method="POST" action="{{ route('review.store', $product) }}">
                        @csrf
                        <div class="mb-3">
                            <input type="text" name="name" class="form-control" placeholder="Votre nom" required style="border-radius:8px;">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <textarea name="comment" class="form-control" rows="4" placeholder="Votre commentaire..." required style="border-radius:8px;"></textarea>
                        </div>
                        <button class="btn w-100 text-white fw-600" style="background:var(--primary);border-radius:8px;">
                            Publier l'avis
                        </button>
                    </form>
                </div>
